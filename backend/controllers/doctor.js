...
export const updateDoctor = (req, res) => {
    const q = "UPDATE `doctor` SET `name`= ?,`qualification`= ?,`contact_no`= ?,`schedule_day`= ?,`schedule_time`= ?,`specialty_id`= ? WHERE id = ?"

    const {id, name, qualification, contact_no, schedule_day, schedule_time, specialty_id} = req.body.values;

    const doctorId = id;

    db.query(q, [name, qualification, contact_no, schedule_day, schedule_time, specialty_id, doctorId], (err, data) => {
        if(err) return res.status(500).json(err);
